...
void USART1_IRQHandler(void){
	...
	Global_Data_Recieved = L_u8msg;
	u8 i=0;
	/*Receive a String [Right Code]*/
	/*(VIP)Uncomment That for recieving string*/
	if((L_u8msg != '\n')&&(L_u8msg != '\r')){
		dd[i]=L_u8msg;
		i++;
	}else if (L_u8msg == '\n'){
		if(strcmp(dd, "omr") == 0){
			MGPIO_voidSetPinValue(MGPIO_PortB ,PIN8 ,MGPIO_PIN_HIGH);
		}
		else if(strcmp(dd, "amr") == 0){
			MGPIO_voidSetPinValue(MGPIO_PortB ,PIN8 ,MGPIO_PIN_LOW);
		}
		i=0;
	}
	dd[i]=L_u8msg;
...
//   {
//	 while(!GET_BIT(uhandler->SR, 5)){
//		 MGPIO_voidWriteData(MGPIO_PortB ,_PIN_8 ,_HIGH);
//	 } MGPIO_voidWriteData(_GPIOB_PORT ,_PIN_8 ,_LOW);
//	 // Wait for data to be received
//
//     data = MUSART_u8ReadData(1);    // Get a character from the USART
